...
import { agendamentos as agendamentosAPI } from "../../service/api";
...
export default function Agenda() {
  ...
  useEffect(() => {
    agendamentosAPI.listar()
      .then(dados => setAgendamentos(dados.map(normalizar)))
      .catch(console.error)
  ...
  // ── Filtros ───────────────────────────────────────────────
  const agendamentosDoDia = agendamentos.filter(a => mesmodia(a.dataISO, dataSelecionada));

  const agendamentosDaSemana = semana.map(data =>
    agendamentos.filter(a => mesmodia(a.dataISO, data))
  );
  ...
  return (
    <div className="p-6 flex gap-6">
      ...
      <div className="flex-1 min-w-0">
        ...
        {carregando && (
          <div className="text-center py-16 text-gray-400">Carregando agenda...</div>
        )}

        {/* ── Visão Diária ── */}
        {!carregando && visualizacao === "Diário" && (
        ...
        {/* ── Visão Semanal ── */}
        {!carregando && visualizacao === "Semanal" && (
          <div>
