...
import { pacientes as pacientesAPI, auth } from '../../service/api'
...
export default function Header() {
  ...
  const sugestoes = busca.trim().length > 0
    ? todosPacientes
        .filter((p) => {
          const nome = p.nome ?? p.paciente?.nome ?? ''
        ...
        {mostrarSugestoes && sugestoes.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-30">
            {sugestoes.map((p, idx) => {
              const nome       = p.nome ?? p.paciente?.nome ?? '—'
              const idade      = p.idade ?? '—'
              const diagnostico = p.diagnostico ?? p.diagnostico ?? ''
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400 truncate">
                      {idade !== '—' ? `${idade} anos` : ''}{diagnostico ? ` · ${diagnostico}` : ''}
                    </p>
                  </div>
                  {status && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${statusColors[status] ?? 'bg-gray-100 text-gray-500'}`}>
                      {status}
                    </span>
                  )}
                </button>
