...
export default function Consultas() {
  ...
  useEffect(() => {
    agendamentosAPI.listar()
      .then(dados => {
        const dataRef = dados[0]?.dataHora ?? dados[0]?.data ?? dados[0]?.horario
        const deHoje = dataRef
          ? dados.filter(a => ehHoje(a.dataHora ?? a.data ?? a.horario))
          : dados
        setConsultas(deHoje.slice(0, 5))
      })
      .catch(console.error)
